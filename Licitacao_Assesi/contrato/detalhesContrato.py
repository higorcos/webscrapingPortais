from Licitacao_Assesi.contrato import tabelasDetalhesContrato
...
def acessarPaginaDetalhes(table,directory,titles,dadosTabela):
    ...
    return dadosTabela
def acessarPaginaDetalhesNovaVersaoJulho2024(links,directory):
    global directoryGlobal
    directoryGlobal = directory

    with concurrent.futures.ThreadPoolExecutor() as executor:
            result = executor.map(mostrarDetalheNovaVersaoJulho2024,links)
            # Iterar sobre os dados em result
    ...
    for res in result:
        for chave, valor in res.items():
            if chave not in meu_dicionario:
                ...
                meu_dicionario[chave].extend(valor)
    print()
    print("\t-------Finalizou Contratos")
...
def mostrarDetalheNovaVersaoJulho2024(link):
    link = link.get('href')
    dadosTabela={}
    chaves=[]

    print("\tAcessando contrato")
    ...
    for input in selectDivs:
        ...
        if (input.find("label")):
            ...
            valor = valor.replace("\n", "").strip() #Remover "\n" e espaço antes de depois do texto
            dadosTabela[chave] = []
            dadosTabela[chave].append(valor)
    ...
    #tabelasDetalhesContrato.verificarSeExisteTabelas(soup, namefolder)
    tabelasDetalhesContrato.verificarTabelaVersaoJulho2024(soup, namefolder)
