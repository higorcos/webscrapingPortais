...
import time
from Licitacao_Assesi.licitacao import detalhesLicitacao
...
def traversev(driver,directoryInformation,link):
    ...
    #numero de paginas
    pagination = soup.select('#tableLicitacoes2_paginate')
    selectNumberPages = pagination[0].find_all('li')
    selectNumberPages.reverse()
    selectNumberPages.pop(0)
    ...
    for i in range(0,numberPages):
        ...
        currentPageLinks = soup.select('#tableLicitacoes2')
        currentPageLinks = currentPageLinks[0].find_all('a')
        ...
        if numberPages != (i+1):
            button = driver.find_element(By.CSS_SELECTOR, "#tableLicitacoes2_next a");
            button.click();
